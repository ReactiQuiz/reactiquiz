// src/pages/AICenterPage.tsx
/**
 * AI Center Page
 * 
 * This page provides an AI chat interface where users can interact
 * with Q, the study assistant. It supports markdown rendering for
 * AI responses and maintains conversation history.
 */
...
/**
 * AI Center Page Component
 * 
 * Displays the AI chat interface with:
 * - Chat message history (user and AI messages)
 ...
 * 
 * This page is only accessible to authenticated users. Messages
 * are sent to the AI API endpoint for processing.
 * 
 * @returns {JSX.Element} AI Center page with chat interface
 */
const AICenterPage: React.FC = () => {
     ...
     * Scroll To Bottom
     * 
     * Scrolls the messages list to the bottom to show the latest message.
     * Used when new messages are added to the conversation.
     */
     ...
     * Auto-Scroll Effect
     * 
     * Automatically scrolls to bottom whenever messages change.
     ...
     * with Material-UI Typography components. Maps markdown elements
     * to MUI Typography variants.
     */
    const markdownComponents = {
        p: ({ node, ...props }: any) => <Typography variant="body1" paragraph sx={{ mb: 1, '&:last-child': { mb: 0 } }} {...props} />,
        strong: ({ node, ...props }: any) => <Typography component="span" fontWeight="bold" {...props} />,
        em: ({ node, ...props }: any) => <Typography component="span" fontStyle="italic" {...props} />,
        ol: ({ node, ...props }: any) => <ol style={{ paddingInlineStart: '20px', margin: 0 }} {...props} />,
        ul: ({ node, ...props }: any) => <ul style={{ paddingInlineStart: '20px', margin: 0 }} {...props} />,
        li: ({ node, ...props }: any) => <li style={{ marginBottom: '4px' }}><Typography component="span" {...props} /></li>,
    };
                                ...
                                <ListItemText
                                    ...
                                    secondary={
                                        <ReactMarkdown components={markdownComponents}>
                                            {msg.parts[0]?.text || 'No message content'}
                                        </ReactMarkdown>
                                    }
