...
export const useResults = (): UseResultsReturn => {
  ...
  // State for filter settings (subject, difficulty, class, genre)
  const [filters, setFilters] = useState<{ subject: string; difficulty: string; class: string; genre: string }>({ 
    ...
    difficulty: 'all', // Filter by difficulty (or 'all' for no filter)
  ...
  const processedResults = useMemo(() => {
    ...
    return allResults.map(result => {
      ...
      return {
        ...result, // Spread existing result data
        // Enrich with topic information or use defaults
   ...
   * Results must match all active filters (subject, difficulty, class, genre).
  ...
  const sortedResults = useMemo(() => {
    const sorted = [...filteredResults]; // Create copy to avoid mutating original
