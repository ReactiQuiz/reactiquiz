...
const fetchQuestionsForSubject = async (tx: Tx, subjectKey: string, totalNeeded: number, difficultyRange: DifficultyRange): Promise<Question[]> => {
...
interface HomiBhabhaParams {
    difficulty: string;
    questionComposition: QuestionComposition;
}

const assembleHomiBhabhaPracticeTest = async (tx: Tx, params: HomiBhabhaParams): Promise<Question[]> => {
...
export { assembleHomiBhabhaPracticeTest };
