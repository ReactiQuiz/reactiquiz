...
router.get('/', asyncHandler(async (req, res) => {
    ...
    try {
        const result = await tx.execute(
            `SELECT t.*, s.subjectKey, s.name as subjectName, s.accentColorDark, s.accentColorLight 
             FROM quiz_topics t 
...
router.get('/:subjectKey', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const topicsResult = await tx.execute({
            sql: `SELECT t.*, s.subjectKey, s.name as subjectName, s.accentColorDark, s.accentColorLight 
                  FROM quiz_topics t 
                  LEFT JOIN subjects s ON t.subject_id = s.id 
                  WHERE t.subject_id = ? 
                  ORDER BY t.name`,
