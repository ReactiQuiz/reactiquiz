 ...
 * Friends Routes
 * 
 * Handles friend requests and friendship management between users.
 * Supports sending, accepting, rejecting, and listing friend requests.
 ...
 * Returns list of pending requests with requester username.
 * Requires authentication via verifyToken middleware.
 */
router.get('/requests/pending', verifyToken, asyncHandler(async (req, res) => {
 ...
 * Updates friendship status to 'accepted' or 'declined' based on action.
 * Requires authentication via verifyToken middleware.
 * 
 * @param {string} requestId - Friend request ID to update
 * @body {string} action - Action to perform ('accept' or 'decline')
 */
router.put('/request/:requestId', verifyToken, asyncHandler(async (req, res) => {
    const userId = req.user.id;
 ...
 * GET /api/friends
 * 
 * Retrieves all accepted friends for the authenticated user.
 * Returns list of friends with friend ID and username.
 * Requires authentication via verifyToken middleware.
 */
...
/**
 * DELETE /api/friends/unfriend/:friendUserId
 * 
 * Removes a friendship between the authenticated user and another user.
 * Deletes the friendship record regardless of who initiated the friendship.
 * Requires authentication via verifyToken middleware.
 * 
 * @param {string} friendUserId - ID of the user to unfriend
 */
router.delete('/unfriend/:friendUserId', verifyToken, asyncHandler(async (req, res) => {
