...
router.post('/request', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await tx.rollback();
        logError('DB ERROR', 'Sending friend request failed', e && e.message);
...
router.get('/requests/pending', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await tx.rollback();
        logError('DB ERROR', 'Fetching pending requests failed', e && e.message);
...
router.put('/request/:requestId', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await tx.rollback();
        logError('DB ERROR', `Updating friend request ${requestId} failed`, e && e.message);
...
router.get('/', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await tx.rollback();
        logError('DB ERROR', 'Fetching friends list failed', e && e.message);
...
router.delete('/unfriend/:friendUserId', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await tx.rollback();
        logError('DB ERROR', `Unfriending failed`, e && e.message);
