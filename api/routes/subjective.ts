...
router.get('/paper/:topicId', verifyToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    const { topicId } = req.params;
    const tx = await turso.transaction('read');
    try {
        const [topicRes, questionsRes] = await Promise.all([
            tx.execute({ sql: "SELECT id, name FROM quiz_topics WHERE id = ?", args: [topicId as string] }),
            tx.execute({ sql: "SELECT id, topic_id, question_text, marks, difficulty FROM subjective_questions WHERE topic_id = ?", args: [topicId as string] })
        ]);
        await tx.commit();
...
router.post('/submit', verifyToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        const questionsMap = new Map(questionsRes.rows.map((q: any) => [q.id, { marks: q.marks, expected_answer: q.expected_answer }]));
        const totalMaxMarks = questionsRes.rows.reduce((sum: number, q: any) => sum + q.marks, 0);

        const initialResultData = answers.map((ans: any) => ({
        ...
        (async () => {
            ...
            try {
                ...
                logError('DB ERROR', 'Failed to save final grades', (e as Error).message);
            }
        })();
...
router.get('/results', verifyToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    const userId = req.user!.id;
    const tx = await turso.transaction('read');
    try {
...
router.get('/results/:resultId', verifyToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
