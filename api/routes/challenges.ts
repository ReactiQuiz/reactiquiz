...
router.post('/', verifyToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        await tx.rollback();
        logError('DB ERROR', 'Creating challenge failed', (e as Error).message);
        res.status(500).json({ message: 'Failed to create challenge.' });
    }
});

router.get('/pending', verifyToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        await tx.rollback();
        logError('DB ERROR', 'Fetching pending challenges failed', (e as Error).message);
