 ...
 */
router.get('/questions/:topicId', verifyToken, async (req, res) => {
...
router.get('/subjective/:topicId', verifyToken, async (req, res) => {
    const { topicId } = req.params;
    const { 
        includeAnswers = 'false',
        pageSize = 'a4',
 ...
 * @query {string} [orientation='portrait'] - PDF orientation
 */
router.get('/quiz-session/:sessionId', verifyToken, async (req, res) => {
 ...
 * @query {string} [orientation='portrait'] - PDF orientation
 */
router.get('/homi-bhabha/:class/:difficulty', verifyToken, async (req, res) => {
