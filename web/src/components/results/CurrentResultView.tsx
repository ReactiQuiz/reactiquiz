// src/components/results/CurrentResultView.tsx
/**
 * Current Result View Component
 * 
 * This component displays the current quiz result view after completing
 * a quiz. It shows the quiz result summary, question breakdown, and
 * action buttons for navigating or viewing history.
 */
 ...
 * Current Result View Component
 * 
 * Displays the current quiz result view with:
 * - Quiz result summary (score, percentage, time taken)
 * - Question breakdown (detailed questions with answers)
 * - Action buttons (View History, Home, Challenge Setup)
 * - Metadata display (subject, topic, class, difficulty)
 * - Empty state handling
 * 
 * This component is used on the ResultsPage to display
 * the result of the most recently completed quiz.
 * 
 * @param {Object} props - Component props
 * @param {Object} props.currentQuizDataFromState - Current quiz data from state
 * @param {Function} props.onViewHistory - Callback to view history
 * @param {Function} props.onNavigateHome - Callback to navigate home
 * @param {Function} props.onOpenChallengeSetup - Callback to open challenge setup
 * @param {Object} props.currentUser - Current user object
 * @returns {JSX.Element} Current quiz result view
 */
          ...
          <Stack spacing={0.5}>
            ...
            {challengeDetails.status === 'completed' && (
                <Typography sx={{ fontWeight: 'bold', mt: 1, color: challengeDetails.winner_id === null ? theme.palette.info.main : (challengeDetails.winner_id === currentUser?.id ? theme.palette.success.main : theme.palette.error.main)}}>
                    {challengeDetails.winner_id === null ? "It's a Tie!" : (challengeDetails.winner_id === currentUser?.id ? "You Won the Challenge!" : `${challengeDetails.winnerUsername || 'Opponent'} Won.`)}
                </Typography>
            )}
