 ...
 * 
 * This component displays a modal dialog for user login and registration.
 * It handles device change detection, OTP verification, and authentication
 * with username and recovery email.
 */
 ...
 * Login Modal Component
 * 
 * Displays a modal dialog for login/registration. Features:
 * - Username and recovery email input fields
 * - Device change detection and OTP verification
 * - Multi-stage flow (login attempt -> OTP request -> OTP verification)
 * - Form validation (email format, OTP format)
 * - Loading states during submission
 * - Error/success message display
 * 
 * This component handles the scenario where a user tries to login
 * from a different device and requires OTP verification.
 * 
 * @param {Object} props - Component props
 * @param {boolean} props.open - Whether the modal is open
 * @param {() => void} props.onClose - Callback to close the modal
 * @param {() => void} props.onLogin - Callback for login attempt
 * @param {string} props.accentColor - Accent color for buttons
 * @param {string} [props.authError] - Authentication error message (optional)
 * @param {(error: string) => void} [props.setAuthError] - Setter for auth error (optional)
 * @param {Object} props.deviceChangeInfo - Device change information object
 * @param {(info: Object) => void} [props.setDeviceChangeInfo] - Setter for device change info (optional)
 * @param {(identifier: string) => Promise<void>} props.requestDeviceChangeOtp - Callback to request OTP
 * @param {(identifier: string, otp: string) => Promise<void>} props.confirmDeviceWithOtp - Callback to confirm with OTP
...
function LoginModal({ 
  ...
  // State for local form errors
  const [localFormError, setLocalFormError] = useState('');
  // Note: emailSentMessage is now deviceChangeInfo.otpSentMessage from App.js
