// src/components/account/UserActivityChart.tsx
/**
 * User Activity Chart Component
 * 
 * This component displays a GitHub-style activity heatmap chart
 * showing user quiz activity over the past year. It displays a
 * grid of squares representing weeks and days with color intensity
 * based on activity count.
 */
import React, { useMemo, useRef, useEffect, useState } from 'react';
import { Box, Typography, Tooltip, useTheme, alpha } from '@mui/material';
import { format, startOfWeek, addDays, getMonth, getDay, parseISO, isSameDay } from 'date-fns';
import useMediaQuery from '@mui/material/useMediaQuery';

/**
 * Day Labels
 * 
 * Full day labels for the week and abbreviated labels for display.
 */
const DAYS_OF_WEEK_LABELS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
const DAY_LABELS_TO_DISPLAY_SHORT = ['Mon', 'Wed', 'Fri'];

/**
 * Chart Layout Constants
 * 
 * Square sizes, margins, and label dimensions for the activity chart.
 */
const SQUARE_SIZE_XS = 11; // Square size for extra small screens
const SQUARE_SIZE_SM_UP = 13; // Square size for small screens and up
const SQUARE_MARGIN = 1.5; // Margin on each side (visual gap is SQUARE_MARGIN * 2)
const MONTH_LABEL_HEIGHT = 20; // Height for month labels in pixels
const DAY_LABEL_WIDTH = 25; // Width for day label column in pixels

/**
 * Get Color For Count
 * 
 * Returns a color based on activity count for the heatmap.
 * Colors become more intense as activity count increases.
 * 
 * @param {number} count - Activity count for a day
 * @param {Object} theme - Material-UI theme
 * @param {string} accentColorInput - Accent color for the chart
 * @returns {string} Color value for the activity square
 */
const getColorForCount = (count, theme, accentColorInput) => {
  const baseColor = accentColorInput || theme.palette.primary.main;
  if (count === 0) return alpha(theme.palette.text.disabled, 0.20);
  if (count <= 2) return alpha(baseColor, 0.35);
  if (count <= 4) return alpha(baseColor, 0.60);
  if (count <= 7) return alpha(baseColor, 0.85);
 ...
 * User Activity Chart Component
 * 
 * Displays a GitHub-style activity heatmap chart with:
 * - Year-long activity grid (53 weeks)
 * - Color intensity based on quiz activity count
 * - Month labels
 * - Day labels (abbreviated)
 * - Tooltips showing activity details
 * - Responsive layout (adjusts visible weeks)
 * - Automatic week calculation based on container width
 * 
 * This component is used on the AccountPage to visualize
 * user quiz activity over time.
 * 
 * @param {Object} props - Component props
 * @param {Array} props.activityData - Array of activity data objects with date and count
 * @param {string} props.accentColor - Accent color for the chart
 * @returns {JSX.Element} Activity heatmap chart
 */
function UserActivityChart({ activityData, accentColor }) {
  // Get theme for styling
  const theme = useTheme();
  // Ref for the main chart container (excluding day labels)
  const chartContainerRef = useRef(null);
  // State for number of visible weeks (default to full year)
  const [visibleWeeksCount, setVisibleWeeksCount] = useState(53);
   ...
   * 
   * Calculates the full year's chart data (53 weeks) from activity data.
   * Maps activity dates to weeks and days, calculates colors,
   * and determines month label positions.
   */
