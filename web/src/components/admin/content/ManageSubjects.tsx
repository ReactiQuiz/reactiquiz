// src/components/admin/content/ManageSubjects.tsx
/**
 * Manage Subjects Component
 * 
 * This component provides a table-based interface for managing subjects
 * in the admin panel. It supports viewing, editing, adding, and deleting
 * subjects with inline editing capabilities.
 */
 ...
 * Manage Subjects Component
 * 
 * Displays a subjects management interface with:
 * - Subjects table display
 * - Edit mode toggle
 ...
 * 
 * @returns {JSX.Element} Subjects management interface
 */
...
function ManageSubjects() {
  ...
  const handleSaveChanges = async () => {
    setIsSaving(true);
    try {
      await apiClient.post('/api/admin/subjects/batch-import', subjects);
  ...
  const handleAddSubject = async () => {
      setIsSaving(true);
      const displayOrderVal = parseInt(String(newSubject.displayOrder || subjects.length + 1), 10);
      const payload = {
        ...newSubject,
        displayOrder: isNaN(displayOrderVal) || displayOrderVal < 1 ? subjects.length + 1 : displayOrderVal
      };
      try {
  ...
  return (
    <Paper variant="outlined">
      <Box sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1.5 }}>
        <Typography variant="h6" sx={{ fontWeight: 700 }}>Manage Subjects</Typography>
