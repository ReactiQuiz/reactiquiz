// src/components/admin/content/ManageTopics.tsx
/**
 * Manage Topics Component
 * 
 * This component provides a table-based interface for managing topics
 * in the admin panel. It supports viewing, editing, adding, and deleting
 * topics with filtering, sorting, and pagination.
 */
 ...
 * Manage Topics Component
 * 
 * Displays a topics management interface with:
 * - Topics table display with pagination
 * - Filter controls (subject, class, genre)
 * - Edit mode toggle
 ...
 * 
 * @returns {JSX.Element} Topics management interface
 */
...
function ManageTopics() {
  ...
  const handleSaveChanges = async () => {
    setIsSaving(true);
    try {
      await apiClient.post('/api/admin/topics/batch-import', topics);
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 1.5 }}>
        <Typography variant="h6" sx={{ fontWeight: 700 }}>Manage Topics</Typography>
        <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            {isEditMode && <Button variant="contained" size="small" startIcon={isSaving ? <CircularProgress size={18} color="inherit"/> : <SaveIcon/>} onClick={handleSaveChanges} disabled={isSaving}>Save All</Button>}
