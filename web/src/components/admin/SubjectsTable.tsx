// src/components/admin/SubjectsTable.tsx
/**
 * Subjects Table Component
 * 
 * This component displays a table for managing subjects in the admin panel.
 * It supports CRUD operations (Create, Read, Update, Delete) with inline
...
/**
 * New Subject Initial State
 * 
 * Default values for a new subject form.
 */
 ...
 * Valid Icon Names
 * 
 * List of valid icon names that can be used for subjects.
 */
 ...
 * Subjects Table Component
 * 
 * Displays a table for managing subjects with:
 * - Subject list display (sorted by display order)
 * - Edit mode toggle
 * - Inline editing for each row
 * - Add new subject row (when edit mode is enabled)
 * - Validation (required fields, unique constraints)
 * - CRUD operations (Create, Read, Update, Delete)
 * - Loading and error states
 * 
 * This component is used on the admin General page to manage
 * subjects in the system.
 * 
 * @returns {JSX.Element} Subjects management table
 */
function SubjectsTable() {
    ...
    const handleSave = async (subjectId, updatedData) => {
        ...
        try {
            ...
            const savedSubject = response.data?.subject || { ...updatedData, id: subjectId };
            const updatedSubjects = subjects.map(s => s.id === subjectId ? savedSubject : s);
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            const addedSubject = response.data?.subject || { ...newSubjectData, id: newSubjectData.subjectKey };
            setSubjects([...subjects, addedSubject]); // Add new subject to the list
