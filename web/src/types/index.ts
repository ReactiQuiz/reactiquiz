...
/**
 * Topic Note Interface
 * 
 * Represents ready-made study notes for a topic, stored in Markdown format
 * with support for LaTeX math, Mermaid diagrams, and SVGs.
 */
export interface TopicNote {
  id: string;
  topicId: string;
  title: string;
  content: string; // Markdown + LaTeX + Mermaid + SVG
  summary?: string;
...
export interface UseResultsReturn {
  ...
  sortOrder: 'date_desc' | 'date_asc' | 'score_desc' | 'score_asc';
  setSortOrder: (order: 'date_desc' | 'date_asc' | 'score_desc' | 'score_asc') => void;
  availableClasses: string[];
  availableGenres: string[];
  clearFilters: () => void;
}
