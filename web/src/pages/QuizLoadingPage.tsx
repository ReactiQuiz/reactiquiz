...
function QuizLoadingPage() {
    ...
    useEffect(() => {
        const startQuiz = async () => {
            ...
            try {
                // 2. Fetch the quiz data and seed the cache.
                // fetchQuery (not prefetchQuery) is used deliberately: prefetchQuery
                // swallows errors by design (it resolves to undefined on failure), so
                // the catch block below never ran and a dead/expired session silently
                // navigated to the quiz page instead of showing an error.
