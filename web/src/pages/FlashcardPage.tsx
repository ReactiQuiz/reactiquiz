 ...
 * Flashcard Page
 * 
 * This page displays flashcards for studying questions from a topic.
 * Users can navigate between cards, shuffle the deck, and view
 * questions with their correct answers.
 */
import React from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { Box, Typography, Button, CircularProgress, Alert, Stack, Chip } from '@mui/material';
 ...
 * 
 * Displays the flashcard study interface with:
 * - Current flashcard display with flip animation
 * - Navigation buttons (previous/next)
 * - Shuffle cards functionality
 * - Card position indicator (current/total)
 * - Loading states during data fetch
 * - Error message display
 * - Empty state (no flashcards available)
 * - Back navigation to subjects
 * - Subject-themed accent colors
 * 
 * This page is accessible to both authenticated and guest users.
 * Topic ID is retrieved from URL parameters.
 * 
 * @returns {JSX.Element} Flashcard page with study interface
 */
const FlashcardPage: React.FC = () => {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 }, maxWidth: '1200px', margin: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      {subject && subject !== 'default' && (
        <Chip label={subject} size="small" sx={{ mb: 1.5, bgcolor: accentColor, color: (t) => t.palette.getContrastText(accentColor) }} />
      )}
      <Typography variant="h1" sx={{ fontSize: { xs: '1.6rem', sm: '2rem' }, textAlign: 'center', mb: 3 }}>
