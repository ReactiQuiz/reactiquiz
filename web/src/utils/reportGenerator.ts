...
const captureChartWithLightModeStyles = async (element: HTMLElement | null): Promise<string | null> => {
  ...
  // Store original dataset colors to restore after chart rendering
  ...
  try {
    ...
    // Restore original dataset colors to return chart to its original state
...
const captureElementAsImage = async (element: HTMLElement | null): Promise<string | null> => {
  ...
  // Find title element and store original size
  ...
  try {
    ...
    // Temporarily increase title font size for PDF readability
    if (titleElement) {
    ...
    // Restore original title font size and weight
    if (titleElement) {
...
export const generateDashboardPdfReport = async ({
  ...
  try {
    ...
    // --- Section 2: Subject Performance Table (if applicable) ---
