...
import {
  Card, CardContent, Typography, Button, useTheme, alpha, Chip, Box, CardActions
} from '@mui/material';
...
function TopicCard({ topic, onStartQuiz, accentColor }) {
  ...
  const cardStyle = {
    ...
    height: '100%' // Added to make cards in a row take same height if parent allows
  };

  return (
    <Card sx={cardStyle}>
      <CardContent sx={{ p: 2, flexGrow: 1 /* Allow content to grow */ }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5, minHeight: '3.2em', fontSize: '0.85rem' }}> {/* Adjusted minHeight and fontSize */}
          {description}
        </Typography>
      </CardContent>
      <CardActions sx={{ p: 1.5, pt:0, alignSelf: 'stretch' /* Ensure actions take full width */ }}> {/* Added CardActions for better button placement */}
        <Button
          variant="outlined"
          onClick={onStartQuiz}
