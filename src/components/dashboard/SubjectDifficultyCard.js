...
import { Paper, Typography, Box, List, ListItem, ListItemText, LinearProgress, useTheme, Stack } from '@mui/material';
...
function SubjectDifficultyCard({ subjectKey, title, data }) {
    ...
    return (
        // --- START OF FIX: Use Stack to fill height and ensure alignment ---
        <Paper elevation={3} sx={{ p: { xs: 2, sm: 2.5 }, height: '100%', borderTop: `4px solid ${accentColor}` }}>
            <Stack sx={{ height: '100%' }}>
                ...
                <List dense sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-around' }}>
